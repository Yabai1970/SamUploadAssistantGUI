    ...
    if quickie:
        ...
        if len(search_results) == 1:
            ...
            if sorted_results:
                ...
                while True:
                    try:
                        selection = cli_ui.ask_string("Informe o número da entrada correta, 0 para nenhuma, ou insira manualmente o IMDb ID (tt1234567): ")
                    except EOFError:
            ...
            try:
                selection = cli_ui.ask_string("Nenhum resultado encontrado. Informe manualmente o IMDb ID (tt1234567) ou 0 para pular: ")
            except EOFError:
                console.print("\n[red]Exiting on user request (Ctrl+C)[/red]")
                await cleanup()
                reset_terminal()
