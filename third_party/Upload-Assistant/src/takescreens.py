...
from src.win32compat import no_console_kwargs
...
async def disc_screenshots(meta, filename, bdinfo, folder_id, base_dir, use_vs, image_list, ffdebug, num_screens=None, force_screenshots=False):
    ...
    if len(existing_images) >= cutoff and not force_screenshots:
        console.print(f"[yellow]Existem pelo menos {cutoff} imagens na lista de imagens. Pulando screenshots adicionais.")
        return
    ...
    if num_screens == 0 and not force_screenshots:
        console.print('[bold green]Reutilizando screenshots. Nenhum screenshot adicional necessário.')
        return

    if meta['debug'] and not force_screenshots:
        console.print(f"[bold yellow]Salvando screenshots... Total necessário: {screens}, Existente: {total_existing}, Capturar: {num_screens}")
