...
class PTS(COMMON):
    ...
    async def generate_description(self, meta):
        ...
        signature_block = build_signature(meta, style="right", size=1).strip()
        if signature_block:
            description_parts.append(signature_block)
    ...
    async def search_existing(self, meta, disctype):
        ...
        if not mandarin:
            response = input("Aviso: áudio ou legenda em mandarim não encontrados. Deseja continuar com o upload mesmo assim? (y/n): ")
