...
class PTP():
    ...
    async def get_ptp_description(self, ptp_torrent_id, meta, is_disc):
        ...
        if not meta.get('only_id'):
            ...
            if not meta.get('skipit') and not meta['unattended']:
                ...
                edit_choice = input("Digite 'e' para editar, 'd' para descartar ou pressione Enter para manter como está: ")
        ...
        try:
            ...
            if response.get('TotalResults'):  # Search results page
                ...
                if total_results == 0:
                    ...
                    try:
                        selected = cli_ui.ask_choice("Selecione o filme correto:", choices=choices)
    ...
    def get_type(self, imdb_info, meta):
        ...
        if ptpType is None:
            if meta.get('mode', 'discord') == 'cli':
                ...
                ptpType = cli_ui.ask_choice("Selecione o tipo correto", choices=ptpTypeList)
                if ptpType == "Concert":
                    ptpType = "Live Performance"
        return ptpType
    ...
    def get_trumpable(self, sub_langs):
        ...
        opts = cli_ui.select_choices("Selecione todas as opções aplicáveis:", choices=list(trumpable_values.keys()))
        trumpable = []
        for opt in opts:
            ...
            if v is None:
                ...
                trumpable.append(15)
                hc_sub_langs = cli_ui.ask_string("Informe o código de idioma das legendas hardcoded")
            ...
            if loggedIn is True:
                ...
                try:
                    ...
                    if resp['Result'] == "TfaRequired":
                        data['TfaType'] = "normal"
                        data['TfaCode'] = cli_ui.ask_string("2FA obrigatório: informe o código 2FA")
                        loginresponse = session.post("https://passthepopcorn.me/ajax.php?action=login", data=data, headers=headers)
                        await asyncio.sleep(2)
                        resp = loginresponse.json()
                    try:
    ...
    async def fill_upload_form(self, groupID, meta):
        ...
        if meta['hardcoded-subs']:
            ...
            if cli_ui.ask_yes_no("Marcar como trumpável?", default=True):
                ptp_trumpable, ptp_subtitles = self.get_trumpable(ptp_subtitles)
        ...
        if groupID is None:  # If need to make new group
            ...
            if meta.get('youtube', None) is None or "youtube" not in str(meta.get('youtube', '')):
                youtube = "" if meta['unattended'] else cli_ui.ask_string("Não foi possível encontrar trailer no YouTube. Informe um link (ex.: https://www.youtube.com/watch?v=dQw4w9WgXcQ)", default="")
            ...
            while cover is None:
                cover = cli_ui.ask_string("Nenhum pôster foi encontrado. Informe um link para um pôster: \n", default="")
            ...
            while new_data["tags"] == "":
                if meta.get('mode', 'discord') == 'cli':
                    ...
                    new_data["tags"] = console.input("Informe pelo menos uma tag, separadas por vírgula (ex.: action, animation, short):")
