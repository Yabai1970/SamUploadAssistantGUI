...
class BHD():
    ...
    async def edit_desc(self, meta):
        ...
        async with aiofiles.open(desc_path, 'w', encoding='utf-8') as desc:
            ...
            signature_block = build_signature(meta, style="align-right", size=4)
            if signature_block:
                if not signature_block.startswith("\n"):
                    signature_block = "\n" + signature_block
                if not signature_block.endswith("\n"):
                    signature_block += "\n"
                await desc.write(signature_block)
            await desc.close()
        return
    ...
    async def search_existing(self, meta, disctype):
        ...
        if any(phrase in bhd_name.lower() for phrase in (
            ...
            if not meta['unattended'] or (meta['unattended'] and meta.get('unattended_confirm', False)):
                ...
                if cli_ui.ask_yes_no("Deseja enviar mesmo assim?", default=False):
                    pass
                else:
                    meta['skipping'] = "BHD"
                    return []
