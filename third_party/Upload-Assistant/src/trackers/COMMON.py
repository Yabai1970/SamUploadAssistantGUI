...
from src.signature import build_signature
...
class COMMON():
    ...
    async def unit3d_edit_desc(self, meta, tracker, signature, comparison=False, desc_header="", image_list=None):
        ...
        try:
            ...
            screensPerRow = 2
        signature_block = ""
        default_signature = (meta.get('ua_signature') or '').strip()
        if signature and signature.strip() and signature.strip() != default_signature:
            signature_block = signature.strip()
        if not signature_block:
            signature_block = build_signature(meta, style="center", size=20)
        try:
            # If custom signature set and isn't empty, use that instead of the signature parameter
            custom_signature = self.config['TRACKERS'][tracker].get('custom_signature', signature_block)
            if custom_signature != '':
                signature_block = custom_signature
        except Exception as e:
    ...
    async def prompt_user_for_id_selection(self, meta, tmdb=None, imdb=None, tvdb=None, mal=None, filename=None, tracker_name=None):
        ...
        if not meta['unattended']:
            selection = input(f"Deseja usar esses IDs de {tracker_name}? (Y/n): ").strip().lower()
            try:
    ...
    async def ptgen(self, meta, ptgen_site="", ptgen_retry=3):
        ...
        if int(meta.get('imdb_id')) != 0:
            ...
            try:
                ...
                params['url'] = console.input("[red]Informe o link do [yellow]Douban[/yellow]: ")
        else:
