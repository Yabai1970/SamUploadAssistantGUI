...
class FL():
    ...
    async def upload(self, meta, disctype):
        ...
        if meta.get('unattended', False) is False:
            fl_confirm = cli_ui.ask_yes_no("Correto?", default=False)
            if fl_confirm is not True:
                fl_name_manually = cli_ui.ask_string("Informe um nome válido", default="")
                if fl_name_manually == "":
    ...
    async def validate_credentials(self, meta):
        ...
        if vcookie is not True:
            ...
            recreate = cli_ui.ask_yes_no("Entrar novamente e criar uma nova sessão?")
            if recreate is True:
