...
def _default_appdir() -> str:
    if os.name == "nt":
        base = os.environ.get("LOCALAPPDATA") or os.environ.get("APPDATA") or str(Path.home() / "AppData" / "Local")
        return str(Path(base) / "UploadAssistant")
    else:
        return str(Path.home() / ".local" / "share" / "UploadAssistant")

base_dir = os.path.abspath(os.environ.get("UA_BASE_DIR") or _default_appdir())
...
try:
    ...
    if not os.path.exists(os.path.abspath(f"{base_dir}/data/config.py")):
        cli_ui.info(cli_ui.red, "Arquivo de configuração 'config.py' não encontrado.")
        cli_ui.info(cli_ui.red, "Certifique-se de que o arquivo está em:", cli_ui.yellow, os.path.abspath(f"{base_dir}/data/config.py"))
        cli_ui.info(cli_ui.red, "Siga as instruções de configuração: https://github.com/Audionut/Upload-Assistant")
        exit()
    else:
...
async def merge_meta(meta, saved_meta, path):
    """Mescla metadados salvos com os atuais, respeitando regras de sobrescrita."""
        ...
        for key, value in saved_meta.items():
            ...
            if clean_key in overwrite_list:
                if clean_key in meta and meta.get(clean_key) is not None:
                    ...
                    if meta['debug']:
                        console.print(f"Sobrescrevendo {clean_key} com valor de meta:", meta[clean_key])
                else:
...
async def print_progress(message, interval=10):
    """Imprime uma mensagem de progresso a cada `interval` segundos até ser cancelado."""
    try:
        while True:
...
def update_oeimg_to_onlyimage():
    """Atualiza todos os valores img_host_* de 'oeimg' para 'onlyimage' no arquivo de config."""
    ...
    if new_content != content:
        with open(config_path, "w", encoding="utf-8") as f:
            f.write(new_content)
        console.print("[green]Atualizado 'oeimg' para 'onlyimage' e 'oeimg_api' para 'onlyimage_api' em config.py[/green]")
    else:
        console.print("[yellow]Nenhuma ocorrência de 'oeimg' ou 'oeimg_api' para atualizar em config.py[/yellow]")


async def process_meta(meta, base_dir, bot=None):
    """Processa os metadados para cada caminho na fila."""
    if use_discord and bot:
        await send_discord_notification(config, bot, f"Iniciando processo de upload para: {meta['path']}", debug=meta.get('debug', False), meta=meta)
    ...
    if meta['imghost'] is None:
        ...
        try:
            ...
            if result:
                console.print("[red]oeimg agora é onlyimage; sua configuração será atualizada[/red]")
                update_oeimg_to_onlyimage()
        except Exception as e:
            console.print(f"[red]Erro ao verificar hosts de imagem: {e}[/red]")
            return
    ...
    if not meta['unattended']:
        ...
        if str(ua).lower() == "true":
            ...
            console.print("[yellow]Executando em Modo Automático")
    meta['base_dir'] = base_dir
    prep = Prep(screens=meta['screens'], img_host=meta['imghost'], config=config)
    try:
        results = await asyncio.gather(
            prep.gather_prep(meta=meta, mode='cli'),
            return_exceptions=True  # Retorna exceções ao invés de lançá-las
        )
        ...
        console.print(f"Erro em gather_prep: {e}")
    ...
    if meta.get('emby_cat', None) == "movie" and meta.get('category', None) != "MOVIE":
        console.print(f"[red]Categoria incorreta detectada! Esperado 'MOVIE', encontrado: {meta.get('category', None)}[/red]")
        meta['we_are_uploading'] = False
        return
    elif meta.get('emby_cat', None) == "tv" and meta.get('category', None) != "TV":
        console.print("[red]Conteúdo de TV não é suportado no momento[/red]")
        meta['we_are_uploading'] = False
        return

    # Se unattended_confirm e precisamos pegar IDs a partir do nome do arquivo, não retornar cedo para mostrar info do BD
    ...
    if not meta.get('emby', False):
        ...
        if meta['debug']:
            console.print(f"Lista de trackers antes da edição: {meta['trackers']}")
    ...
    while confirm is False:
        try:
            editargs = cli_ui.ask_string("Insira os argumentos a corrigir, ex.: (--tag NTb --category tv --tmdb 12345). Digite `continue` para seguir.")
        except EOFError:
            console.print("\n[red]Saindo a pedido do usuário (Ctrl+C)[/red]")
            await cleanup()
            reset_terminal()
        ...
        if not editargs or not editargs.strip():
            console.print("[yellow]Nenhuma entrada fornecida. Informe argumentos, digite `continue` para prosseguir ou Ctrl+C para sair.[/yellow]")
            continue

        try:
            editargs = tuple(editargs.split())
        except AttributeError:
            console.print("[red]Entrada inválida detectada[/red]")
            confirm = False
            continue
        # Rastreia múltiplas edições
        editargs_tracking = editargs_tracking + editargs
        # Carrega args originais; parse trata duplicatas
        ...
        if meta['debug']:
            console.print(f"Lista de trackers durante a edição: {meta['trackers']}")
        meta['edit'] = True
    ...
    if 'remove_trackers' in meta and meta['remove_trackers']:
        ...
        for tracker in meta['remove_trackers']:
            if tracker in meta['trackers']:
                if meta['debug']:
                    console.print(f"[DEBUG] Teria removido {tracker} encontrado no cliente")
                else:
                    meta['trackers'].remove(tracker)
                    removed.append(tracker)
        if removed:
            console.print(f"[yellow]Removendo trackers já presentes no seu cliente: {', '.join(removed)}[/yellow]")
    if not meta['trackers']:
        console.print("[red]Nenhum tracker restante após remoção.[/red]")
        successful_trackers = 0
        meta['skip_uploading'] = 10

    else:
        console.print(f"[green]Processando {meta['name']} para upload...[/green]")
    ...
    if successful_trackers < int(meta['skip_uploading']) and not meta['debug']:
        console.print(f"[red]Trackers bem-sucedidos insuficientes ({successful_trackers}/{meta['skip_uploading']}). SAINDO........[/red]")
        ...
        console.print(f"Processando {filename} para upload.....")
        ...
        for tracker in ['AZ', 'CZ', 'PHD']:
            ...
            if tracker in meta['trackers'] and meta['frame_overlay'] and upload_status is True:
                ...
                console.print("[yellow]AZ, CZ e PHD não permitem frame overlay. O overlay será desativado para este envio.[/yellow]")
        ...
        progress_task = asyncio.create_task(print_progress("[yellow]Ainda processando, aguarde...", interval=10))
        try:
            ...
            if meta.get('comparison', False):
                    ...
                    try:
                        with open(image_data_file, 'r') as img_file:
                            ...
                            if 'image_list' in image_data and not meta.get('image_list'):
                                ...
                                if meta.get('debug'):
                                    console.print(f"[cyan]Carregados {len(image_data['image_list'])} links de imagem salvos anteriormente")

                            if 'image_sizes' in image_data and not meta.get('image_sizes'):
                                meta['image_sizes'] = image_data['image_sizes']
                                if meta.get('debug'):
                                    console.print("[cyan]Tamanhos de imagem salvos anteriormente carregados")

                            if 'tonemapped' in image_data and not meta.get('tonemapped'):
                                meta['tonemapped'] = image_data['tonemapped']
                                if meta.get('debug'):
                                    console.print("[cyan]Status de tonemapped salvo anteriormente carregado[/cyan]")
                    except Exception as e:
                        console.print(f"[yellow]Não foi possível carregar dados de imagem salvos: {str(e)}")
                ...
                try:
                    if meta['is_disc'] == "BDMV":
                        ...
                        try:
                            ...
                            gc.collect()
                            reset_terminal()
                            raise Exception("Erro durante a captura de screenshots")
                        except Exception as e:
                            await cleanup_screenshot_temp_files(meta)
                            await asyncio.sleep(0.1)
                            await cleanup()
                            gc.collect()
                            reset_terminal()
                            raise Exception(f"Erro durante a captura de screenshots: {e}")
                        ...
                        try:
                            ...
                            try:
                                ...
                                if "workers" in str(e2):
                                    console.print("[red]Problema de max workers; veja https://github.com/Audionut/Upload-Assistant/wiki/ffmpeg---max-workers-issues[/red]")
                                raise e2
                ...
                if isinstance(manual_frames_str, str):
                    ...
                    if meta['debug']:
                        console.print(f"Quadros manuais informados: {manual_frames_count}")
                else:
                ...
                if len(meta.get('image_list', [])) < meta.get('cutoff') and meta.get('skip_imghost_upload', False) is False:
                    ...
                    try:
                        ...
                        reset_terminal()
                        if meta['debug']:
                            console.print("[yellow]Limpando recursos...[/yellow]")
                        gc.collect()
                ...
                if 'image_list' in meta and meta['image_list']:
                    try:
                        ...
                        if meta.get('debug'):
                            console.print(f"[cyan]Salvas {len(meta['image_list'])} imagens em image_data.json")
                    except Exception as e:
                        console.print(f"[yellow]Falha ao salvar dados de imagem: {str(e)}")
        finally:
...
async def cleanup_screenshot_temp_files(meta):
    """Limpa arquivos temporários de screenshots para evitar órfãos em falhas."""
        ...
        try:
            for file in os.listdir(tmp_dir):
                ...
                if os.path.isfile(file_path) and file.endswith((".png", ".jpg")):
                    os.remove(file_path)
                    if meta['debug']:
                        console.print(f"[yellow]Arquivo temporário de screenshot removido: {file_path}[/yellow]")
        except Exception as e:
            console.print(f"[red]Erro ao limpar arquivos temporários de screenshots: {e}[/red]", highlight=False)


async def get_log_file(base_dir, queue_name):
    """
    Retorna o caminho do arquivo de log para o diretório base e nome da fila fornecidos.
    """
...
async def load_processed_files(log_file):
    """
    Carrega a lista de arquivos processados a partir do arquivo de log.
    """
...
async def save_processed_file(log_file, file_path):
    """
    Adiciona um arquivo processado ao log, desduplicando e sempre anexando ao final.
    """
    if os.path.exists(log_file):
        with open(log_file, "r") as f:
            try:
...
def get_local_version(version_file):
    """Extrai a versão local do arquivo version.py."""
    try:
        ...
        if match:
            ...
            console.print("[red]Versão não encontrada no arquivo local.")
            return None
    except FileNotFoundError:
        console.print("[red]Arquivo de versão não encontrado.")
        return None


def get_remote_version(url):
    """Busca a versão mais recente no repositório remoto."""
    try:
        ...
        if response.status_code == 200:
            ...
            if match:
                ...
                console.print("[red]Versão não encontrada no arquivo remoto.")
                return None, None
        else:
            console.print(f"[red]Falha ao obter arquivo de versão remoto. Status: {response.status_code}")
            return None, None
    except requests.RequestException as e:
        console.print(f"[red]Erro ao buscar arquivo de versão remoto: {e}")
        return None, None


def extract_changelog(content, from_version, to_version):
    """Extrai entradas do changelog entre as versões especificadas."""
    patterns_to_try = [
...
async def update_notification(base_dir):
    ...
    if version.parse(remote_version) > version.parse(local_version):
        console.print(f"[red][AVISO] [green]Atualização disponível: v[/green][yellow]{remote_version}")
        console.print(f"[red][AVISO] [green]Versão atual: v[/green][yellow]{local_version}")
        ...
        if verbose and remote_content:
            ...
            if changelog:
                ...
                console.print("[yellow]Changelog não encontrado entre as versões.[/yellow]")

    return local_version


async def do_the_thing(base_dir):
    await asyncio.sleep(0.1)  # Evita corrida
    bot = None
    meta = dict()
    paths = []
    ...
    signature_fields = {
        "ua_signature_text": default_cfg.get("ua_signature_text", ""),
        "ua_signature_link": default_cfg.get("ua_signature_link", ""),
        "ua_signature_subtext": default_cfg.get("ua_signature_subtext", ""),
        "uploader_avatar": default_cfg.get("uploader_avatar", ""),
    }
    ...
    try:
        # Se limpeza for a única operação, usa caminho fictício para satisfazer o parser
        if cleanup_only:
            args_list = sys.argv[1:] + ['dummy_path']
            meta, help, before_args = parser.parse(tuple(' '.join(args_list).split(' ')), meta)
            meta['path'] = None  # Limpa caminho fictício após o parse
        else:
        ...
        if meta.get('cleanup'):
            if os.path.exists(f"{base_dir}/tmp"):
                ...
                console.print("[yellow]Pasta tmp esvaziada com sucesso[/yellow]")
                console.print()
        ...
        if not meta['mkbrr']:
            try:
                ...
                if meta['debug']:
                    console.print("[yellow]Valor de configuração mkbrr inválido; usando False por padrão[/yellow]")
                meta['mkbrr'] = False
        if meta['mkbrr'] and not is_binary:
            console.print("[bold red]O binário do mkbrr não está disponível. Verifique se está instalado corretamente.[/bold red]")
            console.print("[bold red]Revertendo para Torf[/bold red]")
            console.print()
            meta['mkbrr'] = False
        ...
        for path in queue:
            ...
            try:
                ...
                if not path:
                    raise ValueError("A variável 'path' não está definida ou está vazia.")
                ...
                if meta.get('delete_tmp', False) and os.path.exists(tmp_path):
                    try:
                        ...
                        if meta['debug']:
                            console.print(f"[yellow]Diretório temporário limpo com sucesso para {os.path.basename(path)}[/yellow]")
                            console.print()
                    except Exception as e:
                        console.print(f"[bold red]Falha ao excluir diretório temporário: {str(e)}")
                ...
                if not keep_meta or meta.get('delete_meta', False):
                    if os.path.exists(meta_file):
                        try:
                            os.remove(meta_file)
                            if meta['debug']:
                                console.print(f"[bold yellow]Arquivo de metadados encontrado e excluído: {meta_file}")
                        except Exception as e:
                            console.print(f"[bold red]Falha ao excluir arquivo de metadados {meta_file}: {str(e)}")
                    else:
                        if meta['debug']:
                            console.print(f"[yellow]Nenhum arquivo de metadados encontrado em {meta_file}")

                if keep_meta and os.path.exists(meta_file):
                    with open(meta_file, "r") as f:
                        saved_meta = json.load(f)
                        console.print("[yellow]Arquivo de metadados existente encontrado; contém valores em cache")
                        meta.update(await merge_meta(meta, saved_meta, path))

            except Exception as e:
                console.print(f"[red]Exceção: '{path}': {e}")
                reset_terminal()
            ...
            if use_discord and config['DISCORD'].get('discord_bot_token') and not meta['debug']:
                if (config.get('DISCORD', {}).get('only_unattended', False) and meta.get('unattended', False)) or not config.get('DISCORD', {}).get('only_unattended', False):
                    try:
                        console.print("[cyan]Iniciando inicialização do bot do Discord...")
                        ...
                        try:
                            ...
                            console.print("[green]Bot do Discord pronto!")
                        except asyncio.TimeoutError:
                            console.print("[bold red]O bot não conseguiu conectar dentro do tempo limite.")
                            console.print("[yellow]Prosseguindo sem integração com o Discord...")
                            if 'connect_task' in locals():
                                connect_task.cancel()
                    except discord.LoginFailure:
                        console.print("[bold red]Token do bot do Discord inválido. Verifique sua configuração.")
                    except discord.ClientException as e:
                        console.print(f"[bold red]Exceção do cliente do Discord: {e}")
                    except Exception as e:
                        console.print(f"[bold red]Erro inesperado durante a inicialização do bot do Discord: {e}")

            if meta['debug']:
                start_time = time.time()

            console.print(f"[green]Buscando informações de {os.path.basename(path)}")
            ...
            if 'we_are_uploading' not in meta or not meta.get('we_are_uploading', False):
                if not meta.get('site_check', False):
                    if not meta.get('emby', False):
                        console.print("O upload não será realizado.......")
                    if 'queue' in meta and meta.get('queue') is not None:
                        processed_files_count += 1
                        if not meta.get('emby', False):
                            skipped_files_count += 1
                            console.print(f"Processados {processed_files_count}/{total_files} arquivos, com {skipped_files_count} ignorados no envio.")
                        else:
                            console.print(f"[cyan]Processados {processed_files_count}/{total_files}.")
                ...
                console.print()
                console.print("[yellow]Processando envios para os rastreadores.....")
                ...
                if 'queue' in meta and meta.get('queue') is not None:
                    ...
                    if 'limit_queue' in meta and int(meta['limit_queue']) > 0:
                        console.print(f"[cyan]Envio concluído: {processed_files_count - skipped_files_count} de {meta['limit_queue']} dentro do limite, de {total_files} arquivos.")
                    else:
                        console.print(f"[cyan]Envio concluído: {processed_files_count - skipped_files_count}/{total_files} arquivos.")
                    if not meta['debug'] or "debug" in os.path.basename(log_file):
                        if log_file:
                            await save_processed_file(log_file, path)
                    await asyncio.sleep(0.1)
                    if sanitize_meta:
                        try:
                            await asyncio.sleep(0.2)  # Não disputar com prints de status
                            meta = await clean_meta_for_export(meta)
                        except Exception as e:
                            console.print(f"[red]Erro ao limpar metadados para exportação: {e}")
                    await cleanup()
                    gc.collect()
                    reset_terminal()
            ...
            if meta['debug']:
                ...
                console.print(f"Uploads processados em {finish_time - start_time:.4f} segundos")
                                ...
                                if 'torrent_id' in status:
                                    ...
                                    if (
                                        ...
                                        if 'skipping' in status and not status['skipping']:
                                            discord_message += f"{tracker} não forneceu mensagem útil.\n"
                            except Exception as e:
                                discord_message += f"Erro ao imprimir dados de {tracker}: {e}\n"
                        discord_message += "Todos os envios para trackers processados.\n"
                        await send_discord_notification(config, bot, discord_message, debug=meta.get('debug', False), meta=meta)
                    except Exception as e:
                        console.print(f"[red]Erro no loop de impressão de rastreadores: {e}[/red]")
                else:
                    await send_discord_notification(config, bot, f"Finalizado o upload: {meta['path']}\n", debug=meta.get('debug', False), meta=meta)
            ...
            if find_requests and meta['trackers'] not in ([], None) and not (meta.get('site_check', False) and 'is_disc' not in meta):
                console.print("[green]Pesquisando solicitações nos rastreadores compatíveis.....")
            ...
            if meta.get('site_check', False):
                if 'queue' in meta and meta.get('queue') is not None:
                    processed_files_count += 1
                    skipped_files_count += 1
                    console.print(f"[cyan]Processados {processed_files_count}/{total_files} arquivos.")
                    if not meta['debug'] or "debug" in os.path.basename(log_file):
                        if log_file:
                            await save_processed_file(log_file, path)
            ...
            if sanitize_meta and not meta.get('emby', False):
                try:
                    await asyncio.sleep(0.3)  # Não disputar com prints de status
                    meta = await clean_meta_for_export(meta)
                except Exception as e:
                    console.print(f"[red]Erro ao limpar metadados para exportação: {e}")
            ...
            if meta.get('delete_tmp', False) and os.path.exists(tmp_path) and meta.get('emby', False):
                try:
                    shutil.rmtree(tmp_path)
                    console.print(f"[yellow]Diretório temporário excluído com sucesso para {os.path.basename(path)}[/yellow]")
                    console.print()
                except Exception as e:
                    console.print(f"[bold red]Falha ao excluir diretório temporário: {str(e)}")
...
async def get_mkbrr_path(meta, base_dir=None):
    try:
        ...
        console.print(f"[red]Erro ao configurar o binário mkbrr: {e}[/red]")
        return None
...
def check_python_version():
    ...
    if int(pyver[0]) != 3 or int(pyver[1]) < 9:
        console.print("[bold red]Versão do Python muito baixa. Utilize Python 3.9 ou superior.")
        sys.exit(1)
...
async def main():
    try:
        ...
        console.print("[red]Tarefas foram canceladas. Saindo com segurança.[/red]")
    except KeyboardInterrupt:
        console.print("[bold red]Programa interrompido. Saindo com segurança.[/bold red]")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        # Usa ProactorEventLoop no Windows para subprocessos
        if sys.platform == "win32":
            asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())

        asyncio.run(main())  # Garante tratamento adequado do loop e limpeza
